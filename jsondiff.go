// Author of this file is https://github.com/nsf
// https://github.com/nsf/jsondiff/blob/master/LICENSE
package stubborn
...
func (ctx *diffContext) newline(s string) {
...
func (ctx *diffContext) key(k string) {
	ctx.buf.WriteString(strconv.Quote(k))
	ctx.buf.WriteString(": ")
}

func (ctx *diffContext) writeValue(v interface{}, full bool) {
	switch vv := v.(type) {
	case bool:
...
func (ctx *diffContext) writeTypeMaybe(v interface{}) {
...
func (ctx *diffContext) writeType(v interface{}) {
	switch v.(type) {
	case bool:
...
func (ctx *diffContext) result(d Difference) {
	if d == NoMatch {
		ctx.diff = NoMatch
...
func (ctx *diffContext) printMismatch(a, b interface{}) {
	ctx.tag(&ctx.opts.Changed)
	ctx.writeMismatch(a, b)
}

func (ctx *diffContext) printDiff(a, b interface{}) {
	if a == nil || b == nil {
		if a == nil && b == nil {
...
// this diffContext being a superset means that for each object or array in the
